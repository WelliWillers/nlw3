...
export default {
    ...
    async create(request: Request, response: Response) {
        ...
        const schema = Yup.object().shape({
            ...
            opening_hours: Yup.string().required('Horários de atendimento é um campo obrigatório.'),
            open_on_weekends: Yup.boolean().required('Aberto nos findes é um campo obrigatório.'),
            images: Yup.array(
                Yup.object().shape({
                    path: Yup.string().required('Imagem é um campo obrigatório.'),
                })
            ) 
        })
